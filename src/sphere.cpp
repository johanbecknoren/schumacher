#include "sphere.h"
#include <sstream>
Sphere::Sphere(const real &radius, const Vec3 &position) {
	_radius = radius;
	_position = position;
	createAABB();
}
...
void Sphere::createAABB() {
	Vec3 lLB(_position.x-_radius, _position.y-_radius, _position.z-_radius);
	Vec3 uRF(_position.x+_radius, _position.y+_radius, _position.z+_radius);
...
bool Sphere::getIntersectionPoint(Ray &ray, IntersectionPoint &ip) const {
    ...
    if(intersecting) {// Intersection with AABB exists
		...
		//ray.print();
		Vec3 intP = ray.getOrigin() + dir * t + real(0.0001) * ray.getDirection();
		Vec3 surfNormal = glm::normalize(intP - _position);
