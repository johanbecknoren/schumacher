...
The recursive nature of this algorithm is evident and it is clear that an exact solution is cannot be found. We instead use a Monte Carlo estimator as presented in \cite{dutre}.
...
Since a (Lambertian) diffuse surface per definition reflects equal amounts of radiance in every direction, the BRDF for such a surface can be treated as a constant. We define the BRDF for all diffuse surfaces as 
\begin{subequations} \label{eq:diffuse_indirect}
\begin{align} 
BRDF &= color*k_d\pi \\
