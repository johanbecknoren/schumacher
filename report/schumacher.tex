...
In \autoref{ch:introduction}, global lightning models are described.
...
Axis aligned bounding boxes are stored with two coordinates, one describing the lower left front vertex and one describing the upper right back vertex.
When doing intersection tests against a ray, vectors are created from the ray origin to each of the two corners of the AABB, $\mathbf{P_{LB}}$ and $\mathbf{P_{UR}}$ divided by the length of each component of the ray direction vector $\mathbf{d}$ as described in \autoref{eq:aabb_t}.

\begin{subequations} \label{eq:aabb_t}
\begin{align}
t_1 =& (P_{LB}(x) - \mathbf{o}(x)) / \mathbf{d}(x)\\
t_2 =& (P_{UR}(x) - \mathbf{o}(x)) / \mathbf{d}(x)\\
t_3 =& (P_{LB}(y) - \mathbf{o}(y)) / \mathbf{d}(y)\\
t_4 =& (P_{UR}(y) - \mathbf{o}(y)) / \mathbf{d}(y)\\
t_5 =& (P_{LB}(z) - \mathbf{o}(z)) / \mathbf{d}(z)\\
t_6 =& (P_{UR}(z) - \mathbf{o}(z)) / \mathbf{d}(z)
\end{align}
\end{subequations}
...
\begin{wrapfigure}{r}{0.5\textwidth} 
	\begin{center}
		\includegraphics[width=0.48\textwidth]{figures/octreetwod.png} 
	\end{center}
